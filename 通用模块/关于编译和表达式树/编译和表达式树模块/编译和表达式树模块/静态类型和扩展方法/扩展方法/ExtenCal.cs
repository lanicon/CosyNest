...
namespace System
{
    ...
    public static partial class ExtenExpressions
    {
        ...
        /// 如果表达式不可执行，则引发异常
        /// </summary>
        /// <param name="exp">要执行的表达式</param>
        /// <returns>表达式的计算结果</returns>
        public static object? CalValue(this Expression? exp)
            => exp?.CanReduce ?? false ? exp.Reduce() : exp switch
            {
                null => null,
        ...
        /// 计算一个一元运算符表达式的值
        /// </summary>
        /// <param name="ex">待计算的一元运算符表达式</param>
        /// <returns>表达式的计算结果</returns>
        public static object? CalValue(this UnaryExpression ex)
        {
            dynamic? op = ex.Operand.CalValue();
            if (ex.Method != null)
                return ex.Method.Invoke(null, new[] { op });
            return ex.NodeType switch
            {
                ExpressionType.Convert
                => op is IConvertible ? Convert.ChangeType(op, ex.Type) : op,
        ...
        /// 并返回方法的值
        /// </summary>
        /// <param name="exp">需要求值的方法表达式</param>
        /// <returns>表达式的计算结果</returns>
        public static object? CalValue(this MethodCallExpression exp)
            => exp.Method.Invoke(
                exp.Object.CalValue(),
                exp.Arguments.Select(x => x.CalValue()).ToArray());
        #endregion
        #region 针对二元运算符表达式
        #region 会引发异常
        /// <summary>
        /// 计算一个二元运算符表达式的值
        /// </summary>
        /// <param name="exp">待计算的二元运算符表达式</param>
        /// <returns>二元运算符表达式的计算结果</returns>
        public static object? CalValue(this BinaryExpression exp)
        {
            var left = exp.Left.CalValue();
            var right = exp.Right.CalValue();
            return exp.Method == null ?
                ToolExpression.CalBinaryOperators(left!, right, exp.NodeType) :
                exp.Method.Invoke(null, new[] { left, right });
        }
        #endregion
        #region 不会引发异常
        /// <summary>
        ...
        public static (bool CanPerformLeft, object? ValueLeft, bool CanPerformRight, object? ValueRight, bool CanPerform, object? Value) CalValueSafety(this BinaryExpression exp)
        {
            var (CanPerformLeft, ValueLeft) = exp.Left.CalValueSafety();
            var (CanPerformRight, ValueRight) = exp.Right.CalValueSafety();
            if (CanPerformLeft && CanPerformRight)
            {
                var (CanPerform, Value) = Expression.MakeBinary
                    (exp.NodeType, Expression.Constant(ValueLeft), Expression.Constant(ValueRight),
                    exp.IsLiftedToNull, exp.Method, exp.Conversion).
