...
namespace System.Time
{
    ...
    class TimerComplex : ITimer
    {
        ...
        public void Start()
        {
            ...
            {
                #region 用来确定时间差的本地函数
                static TimeSpan Get(DateTimeOffset date)
                ...
                if (delay != null)
                {
                    ...
                    PackTimer = new Timer(x =>
                    {
                        ...
                        if (next == null)
                            Dispose();
                        else PackTimer!.Change(Get(next.Value), period);
                    }, null, Get(delay.Value), period);
                }
            }
        }
        #endregion
        ...
        {
            if (NextDate is null)
                throw new ArgumentNullException(nameof(NextDate));
